...
async function main() {
    await shell("wasm-pack", ["build", "--release", "--target=web"]);
...
async function makeWorkerBundle() {
    ...
    const workerSrc = globMatch1(`${srcWasmDir}/snippets/**/workerHelpers.worker.js`);
    await replaceInFile(workerSrc, `from '../../../'`, `from '../../../mpz_ts_wasm'`);
    const lines = (await fs.readFile(workerSrc, "utf-8")).split("\n");
    const newLines = lines.map((line) => line.replace(`from '../../../'`, `from '../../../mpz_ts_wasm'`));
    ...
    const outputStr = output
        ...
        .join("\n");
    await fs.writeFile(path.resolve(workerSrc, "../workerUrl.js"), [
        `const outputStr = ${JSON.stringify(outputStr)};`,
        `const blob = new Blob([outputStr], { type: 'application/javascript' });`,
        `export default URL.createObjectURL(blob);`,
        "",
    ].join("\n"));
    const workerHelpersSrc = globMatch1(`${srcWasmDir}/snippets/**/workerHelpers.js`);
    await replaceInFile(workerHelpersSrc, `new URL('./workerHelpers.worker.js', import.meta.url)`, `workerUrl`);
    const workerHelpersContent = await fs.readFile(workerHelpersSrc, "utf-8");
    await fs.writeFile(workerHelpersSrc, [`import workerUrl from './workerUrl';`, "", workerHelpersContent, ""].join("\n"));
}
...
async function shell(program, args) {
    const child = spawn(program, args, { stdio: "inherit" });
    await new Promise((resolve, reject) => {
        child.on("exit", (code) => {
            if (code !== 0) {
                reject(new Error(`Failed shell command (code=${code}): ${[program, ...args].join(" ")}`));
            }
            else {
                resolve();
            }
